...
| Move narration                    | `game-move`                                                                                                             | `metadata.target_age`, destination `world.locations[].name`, destination `world.locations[].description`, destination `world.characters[]` filtered by `location_id` with public summaries (`id`, `first_name`, `last_name`, `sex`, `appearance`, `background`)                                                                                                   | Prior event history filtered to the destination location, plus a computed `has_visited_before` flag    | Destination `world.locations[].location_image_id` attached as the narration part image       | Move prompting explicitly tells the model to acknowledge return visits, stay consistent with prior descriptions, use only the provided destination character summaries when mentioning who is present, and use `sex` for pronoun choice instead of guessing. |
| Search narration                  | `game-search` with role `search`                                                                                        | Shared context: `metadata.target_age` only. Role-specific `search_context`: current location `id`, `name`, `description`, structured `clues` (each `{id, text, role}`), already-revealed clue IDs, next unrevealed clue (with role for tonal calibration), and exhaustion state                                                                                   | Prior event history filtered to the current location                                                   | Nothing                                                                                      | Search uses canonical clue order with ID-based tracking. Clue `role` (direct_evidence, red_herring, supporting_evidence, suspect_elimination) is available to the narrator for calibrating significance.                                                     |
...
| `world.locations[].clues`             | `game-search` canonical clue progression (structured `{id, text, role}`, ID-based tracking of revealed clues, next unrevealed clue); also available to accusation judging via full blueprint context             | Real clue inventory for each searchable location. Roles enable tonal calibration. |
...
- Search narration is grounded by canonical clue progression with structured
  clues (`{id, text, role}`). Clue roles enable the narrator to calibrate
  significance (direct_evidence feels weighty, red_herring feels intriguing
