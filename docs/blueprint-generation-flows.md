...
This repo currently has a deliberate schema split:

- **Blueprint V1**: live gameplay runtime contract in
  `packages/shared/src/blueprint-schema.ts`
- **Blueprint V2**: authoring/generator/evaluator contract in
  `packages/shared/src/blueprint-schema-v2.ts`

In this phase:

- the blueprint generator targets V2
- the evaluator targets V2
- live gameplay runtime still consumes V1
...
These outputs are generated at runtime in Supabase Edge Functions and still use
Blueprint V1 in this phase.
...
starts from each **Blueprint V1 runtime** schema field and lists where that
field is consumed today.
...
Those assets now target Blueprint V2 authoring data, while gameplay runtime
still consumes Blueprint V1.
