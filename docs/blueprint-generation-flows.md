...
| `world.characters[].agendas`                               | Talk private-character context; shapes how the narrator AI filters responses through behavioral directives (self-protection, protect-other, implicate-other, conditional-reveal). Optional `tell` + `trigger` (`always`/`topic_mentioned`/`pressed`) + `trigger_topics` make behavioral tells deliberate and reactive rather than leaked every turn. | Defaults to `[]` (cooperative witness). Gated clues require specific player actions to unlock. Tells surface only when their trigger fires (or, when no trigger is authored, when the player's message touches the agenda's subject). |
