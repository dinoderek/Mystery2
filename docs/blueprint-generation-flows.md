...
| `narrative.starting_knowledge` | `game-start` and `game-get` surface it as structured `state` fields for the in-game notebook: `state.mystery_summary` plus a `summary` on each `state.locations[]` / `state.characters[]` entry (matched by id). It is no longer formatted into narration. | Structured object with `mystery_summary`, `locations[]` (referencing `world.locations[].id`), and `characters[]` (referencing `world.characters[].id`). Optional; fields are `null` when absent. |
