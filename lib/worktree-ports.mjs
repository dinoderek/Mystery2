...
/**
 * Map a worktree leaf name to its Supabase/Docker project_id.
 *
 * This is the SINGLE SOURCE OF TRUTH for the derivation: it is written into
 * `supabase/config.toml` (and thus becomes the running containers'
 * `com.supabase.cli.project` label), and the worktree GC must derive the same
 * value to recognise a live stack. Trim any trailing hyphen left by the 16-char
 * slice — Docker project names are used as container hostnames, and a DNS label
 * may not end with '-'.
 */
export function worktreeProjectId(worktreeName) {
  return `${BASE_PROJECT_ID}-wt-${worktreeName.slice(0, 16).replace(/-+$/, "")}`;
}
...
export function resolveWorktreePorts(cwd = process.cwd()) {
  ...
  return {
    projectId: worktreeProjectId(worktreeName),
    ports,
    isWorktree: true,
    worktreeName,
    slot,
