...
export function resolveWorktreePorts(cwd = process.cwd()) {
  ...
  return {
    // Trim any trailing hyphen left by the 16-char slice: Docker project names
    // are used as container hostnames, and a DNS label may not end with '-'.
    projectId: `${BASE_PROJECT_ID}-wt-${worktreeName.slice(0, 16).replace(/-+$/, "")}`,
    ports,
    isWorktree: true,
    worktreeName,
    slot,
