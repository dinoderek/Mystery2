...
- Updates `game_sessions.mode = 'accuse'`
- AI generates showdown scene narration prompting for reasoning
- Appends `accuse_start` event

---

## 10. `game-accuse-reasoning` — Submit Accusation Reasoning

**Maps to**: `POST /api/game/{game_id}/accuse/reasoning`  
**Method**: `POST`  
**Auth**: None  
**Valid mode**: `accuse`
...
**Implementation notes**:
- Fetches conversation history for the accusation (prior `accuse_reasoning` events)
- AI provider (mock in this feature) evaluates reasoning against `ground_truth` (available server-side)
- Decision: if reasoning is complete and addresses all key facts → resolve with `win` or `loss`; otherwise → return follow-up question, outcome remains `null`
- On resolution: updates `game_sessions.mode = 'ended'`, sets `outcome`
- Appends `accuse_reasoning` event

### Error `400`
```json
{ "error": "Not in accuse mode" }
```
