...
Notes:
- Prefer `.env.images.local` for live generation, with `OPENROUTER_API_KEY` and optional `OPENROUTER_IMAGE_MODEL`.
- Shell env overrides the file when needed; `.env.local` remains a fallback only.
