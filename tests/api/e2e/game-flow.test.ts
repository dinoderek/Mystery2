...
describe("Full E2E API Investigation Flow", () => {
  it("completes full flow with speaker-aware contracts", async () => {
    const headers = {
    ...
    expect(getData.state.history.some((entry: { speaker: { kind: string } }) => entry.speaker.kind === "character")).toBe(true);
    ...
    expect(accuseResolveData.speaker.kind).toBe("narrator");
